...
<template>
  <div class="py-5 px-10 flex justify-between items-center 2xl:px-80 bg-[#c8ee44]">
    <div class="text-3xl">
      <span class="font-bold text-black">KUD</span><span class="font-light text-black">Teratai Biru</span>
    </div>

    <!-- Desktop Navigation -->
    <div v-if="breakpoints.isGreater('lg')" class="space-x-2 flex">
      <router-link to="/" class="font-medium py-1.5 px-3 rounded-full transition hover:bg-white" active-class="text-black bg-white">
        Beranda
      </router-link>
      <router-link to="/tagihan" class="font-medium py-1.5 px-3 rounded-full transition hover:bg-white" active-class="text-black bg-white">
        {{ isAdmin ? 'Daftar Tagihan' : 'Tagihan' }}
      </router-link>

      <router-link v-if="!isAdmin" to="/rekap" class="font-medium py-1.5 px-3 rounded-full transition hover:bg-white" active-class="text-black bg-white">
        Rekap
      </router-link>

      <router-link v-else to="/nasabah" class="font-medium py-1.5 px-3 rounded-full transition hover:bg-white" active-class="text-black bg-white">
        Daftar Nasabah
      </router-link>

      <router-link to="/pengajuan" class="font-medium py-1.5 px-3 rounded-full transition hover:bg-white" active-class="text-black bg-white">
        {{ isAdmin ? 'Daftar Pengajuan' : 'Pengajuan' }}
      </router-link>
      <router-link to="/bantuan" class="font-medium py-1.5 px-3 rounded-full transition hover:bg-white" active-class="text-black bg-white">
        Bantuan
      </router-link>
        ...
        <div class="flex items-center gap-x-3">
          ...
          <Menu as="div" class="text-left relative">
            ...
            >
              ...
              >
                <menu-item >
                  <router-link to="/profile" class="group flex rounded-md items-center w-full px-2 py-2 text-sm">
                    Ubah Profile
                  </router-link>
                </menu-item>
                <menu-item >
